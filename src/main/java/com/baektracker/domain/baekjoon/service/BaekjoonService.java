package com.baektracker.domain.baekjoon.service;

import com.baektracker.common.util.TimeUtils;
...
public class BaekjoonService {
    ...
    public void loadBaekjoonProblemStatus() {
        ...
        for (User user : users) {
            ...
            int top = -1;
            do {
                List<SolvedProblem> res = null;
                try {
                    res = scrapProblemPage(user, top, user.getLastRead());
                    if (res.isEmpty()) {
                        break;
                    }
                    ...
                    throw CustomException.of(ApiResponseCode.BAEKJOON_PAGE_SCRAP_ERROR, "", e);
                }
            } while (true);
...
//        Element tbody = element.children().get(1);
        Element tbody = connection.get()
                .getElementsByAttributeValue("id", "status-table")
                .get(0)
                .children()
                .get(1);
