...
public class BaekjoonService {
    ...
    public int loadBaekjoonProblemStatus() {
        ...
        try {
            ...
            for (User user : users) {
                ...
                int top = -1;
                do {
                    List<SolvedAcResponseVO> res = scrapProblemPage(user.getNickname(), top, user.getLastRead());
    ...
    private List<SolvedAcResponseVO> scrapProblemPage(String username, int top, int lastReadIndex) throws IOException {
        if (username == null || username.isEmpty()) {
            return null;
        }
        Connection connection = Jsoup.connect(
                Baekjoon_Problem_Status_Page_URL + "?user_id=" + username + "&top=" + top);
            ...
            for (int i = 0; i < maxLength; ++i) {
                ...
                if (selects != null) {
                    ...
                    if (!problemRepository.existsById(problemId)) {
                        ...
                        BaekjoonProblem newProblem = BaekjoonProblem.builder()
                                .id(problemId)
